<?php
namespace Database\ForOverridden;
 ...
 *
 * \Database\Connection works together with {@link \Database\Command}, {@link CDbDataReader}
 * and {@link \Database\Transaction} to provide data access to various DBMS
 ...
 *
 * The following example shows how to create a \Database\Connection instance and establish
 * the actual connection:
 * <pre>
 * $connection=new \Database\Connection($dsn,$username,$password);
 ...
 * </pre>
 *
 * \Database\Connection also provides a set of methods to support setting and querying
 * of certain DBMS attributes, such as {@link getNullConversion nullConversion}.
 *
 * Since \Database\Connection implements the interface IApplicationComponent, it can
 ...
 * @property \Database\Transaction $currentTransaction The currently active transaction. Null if no active transaction.
 * @property \Database\schema\Schema $schema The database schema for the current connection.
 * @property \Database\schema\CommandBuilder $commandBuilder The command builder.
...
class Connection
{
	 ...
	 * effective when the \Database\Connection object is used as an application component.
	 */
	 ...
	 * By setting this property, any token like '{{tableName}}' in {@link \Database\Command::text} will
	 ...
	 */
	public $driverMap=[
		'pgsql'   => '\Database\schema\pgsql\Schema',// PostgreSQL
		'mysqli'  => '\Database\schema\mysql\Schema',// MySQL
		'mysql'   => '\Database\schema\mysql\Schema',// MySQL
		'sqlite'  => '\Database\schema\sqlite\Schema',// sqlite 3
		'sqlite2' => '\Database\schema\sqlite\Schema',// sqlite 2
		'mssql'   => '\Database\schema\mssql\Schema',// Mssql driver on windows hosts
		'dblib'   => '\Database\schema\mssql\Schema',// dblib drivers on linux (and maybe others os) hosts
		'sqlsrv'  => '\Database\schema\mssql\Schema',// Mssql
		'oci'     => '\Database\schema\oci\Schema',// Oracle driver
	];
	 ...
	 * @param $config
	 * @internal param string $dsn The Data Source Name, or DSN, contains the information required to connect to the database.
	 * @internal param string $username The user name for the DSN string.
	 * @internal param string $password The password for the DSN string.
	 * @see      http://www.php.net/manual/en/function.PDO-construct.php
	 */
	 ...
	 * when the \Database\Connection is used as an application component.
	 ...
	 * @throws \Database\Exception if connection fails
	 */
	 ...
	 * @param  $dependency the dependency that will be used when saving
	 * the query results into cache.
	 * @param integer $queryCount number of SQL queries that need to be cached after calling this method. Defaults to 1,
	 * meaning that the next SQL query will be cached.
	 * @return \Database\Connection the connection instance itself.
	 * @since 1.1.7
	 */
	...
	protected function open()
	{
		if($this->_pdo===null)
		{
			if(empty($this->connectionString))
				throw new DBException('\Database\Connection.connectionString cannot be empty.');
			try
			{
				//echo('Opening DB connection system.db.\Database\Connection');
			...
			catch(PDOException $e)
			{
				...
				{
					throw new DBException('\Database\Connection failed to open the DB connection: '.
						$e->getMessage(),(array)$this,(int)$e->getCode(),$e->errorInfo);
				}
				else
				{
					//Yii::log($e->getMessage(),CLogger::LEVEL_ERROR,'exception.');
					throw new DBException('\Database\Connection failed to open the DB connection.',(int)$e->getCode(),$e->errorInfo);
				}
			}
		}
	}
	...
	{
		//Yii::trace('Closing DB connection','system.db.\Database\Connection');
		$this->_pdo=null;
		$this->_active=false;
		$this->_schema=null;
	}
	...
	protected function createPdoInstance()
	{
		...
		if(!class_exists($pdoClass))
			throw new DBException('\Database\Connection is unable to find PDO class "{className}". Make sure PDO is installed correctly.');
	 ...
	 * or an array representing different fragments of a SQL statement. Please refer to {@link \Database\Command::__construct}
	 * for more details about how to pass an array as the query. If this parameter is not given,
	 * you will have to call query builder methods of {@link \Database\Command} to build the DB query.
	 * @return Command the DB command
	 */
	public function createCommand($query=null)
	{
		$this->setActive(true);
		#!!Magic, Dont Remove class PDO exteds \PDO!
		return new \Database\Command($this,$query);

	}

	/**
	 * Returns the currently active transaction.
	 * @return \Database\Transaction the currently active transaction. Null if no active transaction.
	 */
	 ...
	 * @return \Database\Transaction the transaction initiated
	 */
	public function beginTransaction()
	{
		//Yii::trace('Starting transaction','system.db.\Database\Connection');
		$this->setActive(true);
		$this->_pdo->beginTransaction();
		return $this->_transaction=new \Database\Transaction($this);
	}

	/**
	 * Returns the database schema for the current connection
	 * @throws DBException if \Database\Connection does not support reading schema for specified database driver
	...
	public function getSchema()
	{
		...
		else
		{
			...
			if(isset($this->driverMap[$driver]))
				return $this->_schema = new $this->driverMap[$driver]($this); //Yii::createComponent($this->driverMap[$driver], $this);
			else
				throw new DBException('\Database\Connection does not support reading schema for {driver} database.');
		}
	}

	/**
	 * Returns the SQL command builder for the current DB connection.
	 * @return \Database\schema\CommandBuilder the command builder
	 */
	...
	{
		throw new DBException('#TODO - Database\Connection\getStats');
